...
def _persist(
    ...
    """
    Persist a Plotly figure to both HTML and PNG in the given output directory.

    As a convenience, this function returns a dictionary with the paths to the saved files.

    Args:
        fig (go.Figure): The Plotly figure to save.
        name (str): The base name for the output files (without extension).
        output_dir (str | Path): The directory where the files will be saved.
        width (int, optional): The width of the PNG image. Defaults to 1200.
        height (int, optional): The height of the PNG image. Defaults to 650.

    Returns:
        dict: A dictionary containing the paths to the saved HTML and PNG files.
    """
...
def _base_layout(
    fig: go.Figure, title: str, yaxis_title: str = "", xaxis_title: str = "Date"
) -> go.Figure:
    """
    Apply a consistent layout to a Plotly figure, including template, colorway,
    titles, legend, hovermode, margins, and font settings.

    Args:
        fig (go.Figure): The Plotly figure to update.
        title (str): The title of the figure.
        yaxis_title (str, optional): The title for the y-axis. Defaults to "".
        xaxis_title (str, optional): The title for the x-axis. Defaults to "Date".

    Returns:
        go.Figure: The updated Plotly figure with the applied layout.
    """
    fig.update_layout(
        template=TEMPLATE,
        colorway=COLORWAY,
...
def plot_small_lot_share_regime(
    small_lot_share: pd.Series,
    regime_dates: dict[str, str],
    output_dir: str | Path,
    name: str = "small_lot_share_regime",
) -> tuple[go.Figure, dict]:
    """
    Small-lot call volume share over time with regime-shift annotations.

    Args:
        small_lot_share (pd.Series): Time series of small-lot call volume share.
        regime_dates (dict[str, str]): Dictionary mapping regime labels to dates.
        output_dir (str | Path): Directory to save the output files.
        name (str, optional): Base name for the output files. Defaults to "small_lot_share_regime".

    Returns:
        tuple[go.Figure, dict]: A tuple containing the Plotly figure and a dictionary with paths to the saved HTML and PNG files.
    """
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(
            x=small_lot_share.index,
            y=small_lot_share.values,
            mode="lines",
            line=dict(width=2.5, color=COLORWAY[0]),
            name="Small-lot call volume share (10d MA)",
            fill="tozeroy",
            fillcolor="rgba(11,95,255,0.08)",
        )
    )
    for label, dt in regime_dates.items():
        ...
        fig.add_annotation(
            ...
            showarrow=False,
            yshift=10,
            font=dict(size=11, color=COLORWAY[1]),
        )
    fig = _base_layout(
        fig,
        "Retail Proxy: Small-Lot Call Volume Share Through Time",
...
def plot_rolling_ic_regime(
    rolling_ic: pd.Series,
    regime_dates: dict[str, str],
    output_dir: str | Path,
    name: str = "rolling_ic_regime",
) -> tuple[go.Figure, dict]:
    """
    Rolling information coefficient (IC) over time with a zero line and regime-shift annotations.

    Args:
        rolling_ic (pd.Series): Time series of rolling information coefficient values.
        regime_dates (dict[str, str]): Dictionary mapping regime labels to dates.
        output_dir (str | Path): Directory to save the output files.
        name (str, optional): Base name for the output files. Defaults to "rolling_ic_regime".
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name="Rolling IC (EOV vs fwd return)",
        )
    )
    ...
    fig = _base_layout(
        fig,
        "Signal Regime Shift: Rolling Cross-Sectional Information Coefficient",
...
def plot_equity_curve(
    ...
    Args:
        strategy_returns (pd.Series): Time series of strategy returns.
        benchmark_returns (pd.Series | None): Time series of benchmark returns. If None, no benchmark overlay is plotted.
        output_dir (str | Path): Directory to save the output files.
        name (str, optional): Base name for the output files. Defaults to "equity_curve".
        drawdown (bool, optional): Whether to include a drawdown subplot. Defaults to True.
    ...
    if benchmark_returns is not None:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                name="Benchmark",
                mode="lines",
                line=dict(width=1.5, dash="dot", color=COLORWAY[3]),
            ),
            row=1,
            col=1,
        )
    ...
    fig = _base_layout(
        fig,
        "RFGD Strategy: Cumulative Performance & Drawdown",
...
def plot_signal_decay_ribbon(
    rolling_ic_by_period: pd.DataFrame,
    output_dir: str | Path,
    name: str = "signal_decay_ribbon",
) -> tuple[go.Figure, dict]:
    """
    Multi-year rolling-IC ribbon chart -- one line per calendar-year regime, x-axis = trading day

    Lets a PM see at a glance whether the signal is decaying year over year.

    Args:
        rolling_ic_by_period (pd.DataFrame): DataFrame containing rolling IC values with 'year' and 'day_of_year' columns.
        output_dir (str | Path): Directory to save the output files.
        name (str, optional): Base name for the output files. Defaults to "signal_decay_ribbon".
    ...
    fig = _base_layout(
        fig,
        "Year-over-Year Signal Decay Ribbon (Rolling IC)",
...
def plot_risk_dashboard(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=turnover.values,
            mode="lines",
            line=dict(color=COLORWAY[2], width=1.2),
            name="Turnover",
            fill="tozeroy",
            fillcolor="rgba(0,182,122,0.25)",
        ),
        row=2,
        col=1,
    )
    ...
    fig.add_trace(
        go.Scatter(
            x=kpi_history.index,
            y=kpi_history["gross_exposure"],
            line=dict(color=COLORWAY[4], width=2),
            name="Gross Exp",
        ),
        row=2,
        col=2,
...
def plot_cpcv_fold_diagnostics(
    fold_results: pd.DataFrame,
    output_dir: str | Path,
    name: str = "cpcv_fold_diagnostics",
) -> tuple[go.Figure, dict]:
    ...
    fig.add_trace(
        go.Box(
            y=fold_results["oos_sharpe"],
            name="OOS Sharpe (CPCV folds)",
            boxpoints="all",
            jitter=0.4,
            pointpos=-1.8,
    ...
    fig = _base_layout(
        fig,
        "Combinatorial Purged Cross-Validation: Out-of-Sample Sharpe Distribution",
...
def plot_pbo_histogram(
    rank_logits: np.ndarray,
    pbo_estimate: float,
    output_dir: str | Path,
    name: str = "pbo_histogram",
) -> tuple[go.Figure, dict]:
    ...
    fig = _base_layout(
        ...
        xaxis_title="Logit of relative rank",
    )
    return fig, _persist(fig, name, output_dir)
